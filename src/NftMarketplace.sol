...
contract NftMarketplace is IERC721Receiver{
    ...
    modifier notList(uint token_id){
        require(tokenSeller[token_id]==address(0) && tokenPrice[token_id]==0,"item is in the list!");
        _;
    }

    modifier isList(uint token_id){
        require(tokenSeller[token_id]!=address(0) && tokenPrice[token_id]!=0,"item is NOT in the list!");
        _;
    }

    modifier isOwner(uint token_id){
        require(IERC721(nft).ownerOf(token_id) == msg.sender,"Not item's owner!");
        _;
    }
    ...
    event ItemCanceled(address,uint);
    event NFTSold(address,uint,uint);
    error NotApprovedForMarketplace();

    function cancelListing(uint token_id) external isList(token_id) isOwner(token_id){
        tokenSeller[token_id] = address(0);
        tokenPrice[token_id] = 0;
        emit ItemCanceled(msg.sender, token_id);
    }
